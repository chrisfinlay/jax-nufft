...
def _reference_forward(image: np.ndarray, uvw: np.ndarray, freq: np.ndarray, pixsize_l: float, pixsize_m: float) -> np.ndarray:
...
def test_forward_matches_dft_single_channel_zenith(eps: float, w_strategy: str) -> None:
    ...
    image = (
        rng.standard_normal((1, n_l, n_m)) + 1j * rng.standard_normal((1, n_l, n_m))
    )
